...
def text_formatting(column='id', formatting='lower/upper/capitalize', temp_view_input='temp_view_table', temp_view_output='temp_view_table', spark_session=scSpark):
    ...
    if formatting == 'lower':
        df_final = df_processed.withColumn(column, lower(column))
    elif formatting == 'upper':
        df_final = df_processed.withColumn(column, upper(column))
    elif formatting == 'capitalize':
        df_final = df_processed.withColumn(column, initcap(column))
...
##################################################################################
INPUT_FORMATTING_STRING = (INPUT_COLUMN, INPUT_FORMAT)
COLUMN_NAME = list(INPUT_FORMATTING_STRING)[0]
CHANGE_FORMATTING_TO = list(INPUT_FORMATTING_STRING)[1]

OPERATOR_ID_PREV = 'B001'     # Previous Operator ID
OPERATOR_ID_CURR = 'B002' # Current Operator ID

temp_view_next = text_formatting(column=COLUMN_NAME, 
                                 formatting=CHANGE_FORMATTING_TO, 
