...
def new_constant_column(new_column_name='new', new_column_value='new', temp_view_input='temp_view_table', temp_view_output='temp_view_table', spark_session=scSpark):
    
    temp_view_input = f"{client}_tbl_{temp_view_input}"
    temp_view_output = f"{client}_tbl_{temp_view_output}"

    df_processed = spark_session.sql(f"SELECT * FROM {temp_view_input}")
    df_final = df_processed.withColumn(new_column_name, when(lit(True), lit(new_column_value)))
...
for i in INPUT_COLUMN:
    if i['isNumeric'].lower() == "true":
        try:
            INPUT_VALUE = int(i['constant'])
        except ValueError:
            INPUT_VALUE = float(i['constant'])
        except:
            INPUT_VALUE = "RE-CHECK INPUT VALUE"
    else:
        INPUT_VALUE = i['constant']

    INPUT_NEW_COLUMN = ((i['column'], INPUT_VALUE))
    NEW_COLUMNS = list(INPUT_NEW_COLUMN)

    OPERATOR_ID_PREV = 'B001'     # Previous Operator ID
    OPERATOR_ID_CURR = 'B001' # Current Operator ID

    temp_view_next = new_constant_column(new_column_name=NEW_COLUMNS[0], 
                                     new_column_value=NEW_COLUMNS[1], 
