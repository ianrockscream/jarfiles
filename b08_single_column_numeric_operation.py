...
def single_column_numeric_operation(source_column='source', target_column='target', operation='multiply', single_value=1, temp_view_input='temp_view_table', temp_view_output='temp_view_table', spark_session=scSpark):
    ...
    if operation == 'multiply':
        df_final = df_processed.withColumn(target_column, col(source_column) * single_value)
...
for input in INPUT_COLUMN:
    ...
    TARGET_COLUMN_NAME = input['target']
    OPERATION = input['operation']
    SINGLE_VALUE = input['value']

    OPERATOR_ID_PREV = 'B001'     # Previous Operator ID
    OPERATOR_ID_CURR = 'B001' # Current Operator ID

    temp_view_next = single_column_numeric_operation(source_column=SOURCE_COLUMN_NAME, 
