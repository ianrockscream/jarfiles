...
INPUT_OLD = (sys.argv[10])
print(INPUT_OLD)

INPUT_NEW = (sys.argv[11])
print(INPUT_NEW)
...
def replace_string(column='id', old_string='old', new_string='new', temp_view_input='temp_view_table', temp_view_output='temp_view_table', spark_session=scSpark):
    
    temp_view_input = f"{client}_tbl_{temp_view_input}"
    temp_view_output = f"{client}_tbl_{temp_view_output}"

    df_processed = spark_session.sql(f"SELECT * FROM {temp_view_input}")
    df_final = df_processed.withColumn(column, regexp_replace(column, old_string, new_string))
...
##################################################################################
INPUT_REPLACE_STRING = (INPUT_COLUMN, INPUT_OLD, INPUT_NEW)
COLUMN_NAME = list(INPUT_REPLACE_STRING)[0]
OLD_STRING = list(INPUT_REPLACE_STRING)[1]
NEW_STRING = list(INPUT_REPLACE_STRING)[2]

OPERATOR_ID_PREV = 'B001'     # Previous Operator ID
OPERATOR_ID_CURR = 'B002' # Current Operator ID

temp_view_next = replace_string(column=COLUMN_NAME, 
