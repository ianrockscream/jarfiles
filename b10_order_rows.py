...
def re_ordering_row(re_order_by='*', order='ASC', temp_view_input='temp_view_table', temp_view_output='temp_view_table', spark_session=scSpark):
    
    temp_view_input = f"{client}_tbl_{temp_view_input}"
    temp_view_output = f"{client}_tbl_{temp_view_output}"
    
    ordering_rules = re_order_by.replace(",", f" {order},")
    
    query = f"""
    SELECT *
    FROM {temp_view_input}
    ORDER BY {ordering_rules} {order}
    """
...
##################################################################################
INPUT_ORDER_COLUMNS = ([i.strip() for i in INPUT_COLUMNS.split(",")], (INPUT_ORDER))
ORDER_COLUMNS = ", ".join(list(INPUT_ORDER_COLUMNS[0]))
ORDER = INPUT_ORDER_COLUMNS[1]

OPERATOR_ID_PREV = 'B001'     # Previous Operator ID
OPERATOR_ID_CURR = 'B002' # Current Operator ID

temp_view_next = re_ordering_row(re_order_by=ORDER_COLUMNS, 
                                 order=ORDER, 
