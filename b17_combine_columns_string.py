...
INPUT_COLUMN_FIRST = (sys.argv[9])
print(INPUT_COLUMN_FIRST)

INPUT_COLUMN_SECOND = (sys.argv[10])
print(INPUT_COLUMN_SECOND)

INPUT_COLUMN_TARGET = (sys.argv[11])
print(INPUT_COLUMN_TARGET)

INPUT_OPERATION = (sys.argv[12])
print(INPUT_OPERATION)

INPUT_SEPARATOR = (sys.argv[13])
...
def combine_columns_string(first_source_column='first_source', second_source_column='second_source', target_column='target', operation='concatenate', separator=" ", temp_view_input='temp_view_table', temp_view_output='temp_view_table', spark_session=scSpark):
    ...
    if operation == 'concatenate':
        df_final = df_processed.withColumn(target_column, concat(col(first_source_column), lit(separator), col(second_source_column)))
...
##################################################################################
INPUT_COMBINE_COLUMNS_STRING = (INPUT_COLUMN_FIRST, INPUT_COLUMN_SECOND, INPUT_COLUMN_TARGET, INPUT_OPERATION, INPUT_SEPARATOR)
FIRST_SOURCE_COLUMN_NAME = list(INPUT_COMBINE_COLUMNS_STRING)[0]
SECOND_SOURCE_COLUMN_NAME = list(INPUT_COMBINE_COLUMNS_STRING)[1]
TARGET_COLUMN_NAME = list(INPUT_COMBINE_COLUMNS_STRING)[2]
OPERATION = list(INPUT_COMBINE_COLUMNS_STRING)[3]
SEPARATOR = list(INPUT_COMBINE_COLUMNS_STRING)[4]

OPERATOR_ID_PREV = 'B001'     # Previous Operator ID
OPERATOR_ID_CURR = 'B002' # Current Operator ID

temp_view_next = combine_columns_string(first_source_column=FIRST_SOURCE_COLUMN_NAME, 
