...
def select_column(selected_columns='*', temp_view_input='temp_view_table', temp_view_output='temp_view_table', spark_session=scSpark):
    
    temp_view_input = f"{client}_tbl_{temp_view_input}"
    temp_view_output = f"{client}_tbl_{temp_view_output}"
    
    df_processed = spark_session.sql(f"SELECT * FROM {temp_view_input}")
    df_final = df_processed.select(*selected_columns)
...
##################################################################################
INPUT_SELECTED_COLUMNS = [i.strip() for i in INPUT_COLUMNS.split(",")]
# SELECTED_COLUMNS = ", ".join(INPUT_SELECTED_COLUMNS)

OPERATOR_ID_PREV = 'B001' # Previous Operator ID
OPERATOR_ID_CURR = 'B002' # Current Operator ID

temp_view_next = select_column(selected_columns=INPUT_SELECTED_COLUMNS, 
