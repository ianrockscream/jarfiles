...
def change_column_type(column_name='new', column_type='string', temp_view_input='temp_view_table', temp_view_output='temp_view_table', spark_session=scSpark):
    
    temp_view_input = f"{client}_tbl_{temp_view_input}"
    temp_view_output = f"{client}_tbl_{temp_view_output}"

    df_processed = spark_session.sql(f"SELECT * FROM {temp_view_input}")
    df_final = df_processed.withColumn(column_name, col(column_name).cast(column_type))
...
##################################################################################
INPUT_CHANGE_COLUMN_TYPE = ((INPUT_COLUMN, INPUT_TYPE))
CHANGE_COLUMN_TYPE = list(INPUT_CHANGE_COLUMN_TYPE)

OPERATOR_ID_PREV = 'B001' # Previous Operator ID
OPERATOR_ID_CURR = 'B002' # Current Operator ID

temp_view_next = change_column_type(column_name=CHANGE_COLUMN_TYPE[0], 
                                    column_type=CHANGE_COLUMN_TYPE[1], 
