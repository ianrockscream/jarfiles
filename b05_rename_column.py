...
def rename_column(renamed_columns='*', temp_view_input='temp_view_table', temp_view_output='temp_view_table', spark_session=scSpark):
    
    temp_view_input = f"{client}_tbl_{temp_view_input}"
    temp_view_output = f"{client}_tbl_{temp_view_output}"

    df_processed = spark_session.sql(f"SELECT * FROM {temp_view_input}")
    df_final = df_processed.withColumnRenamed(renamed_columns[0], renamed_columns[1])
...
##################################################################################
INPUT_RENAMED_COLUMNS = json.loads(INPUT_COLUMN)
print(INPUT_RENAMED_COLUMNS)

for i in INPUT_RENAMED_COLUMNS:
    RENAMED_COLUMNS = [i['column'],i['renameColumn']]
    print(RENAMED_COLUMNS)

    OPERATOR_ID_PREV = 'B001'     # Previous Operator ID
    OPERATOR_ID_CURR = 'B001' # Current Operator ID

    temp_view_next = rename_column(renamed_columns=RENAMED_COLUMNS, 
